...
const createDefaultLogger = ({outputMode, ...options}) => pino({
  ...options,
  prettyPrint: outputMode === 'json' ? false : {forceColor: true},
...
export default ({
  ...
  const runExample = ({local = 'local.json'}) => {
    ...
    const log = createLogger({name, example, level, outputMode})
