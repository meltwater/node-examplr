...
export default ({
  ...
  const runExample = ({local = 'local.json'}) => {
    ...
    const outputMode = options.logOutputMode || 'pretty'
    const logFilterName = options.logFilter

    if (logFilterName && !logFilters.hasOwnProperty(logFilterName)) {
      throw new Error(`No such log filter ${logFilterName}.`)
    }
    ...
    const log = createLogger({
      ...
      filter: logFilterName ? logFilters[logFilterName] : null
    })
